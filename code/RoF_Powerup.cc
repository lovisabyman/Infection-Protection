...
RoF_powerup::RoF_powerup(int xpos)
...
//at Linköpings university. https://gitlab.ida.liu.se/TDDI82/SFML-exempel
{
    ...
    //Baserat på bildens storlek och placering.
    position.top    = reset;
...
bool RoF_powerup::handle_collision(Object* next,
    ...
    else if (dynamic_cast<Projectile*>(next))
    {
        ...
        for (unsigned int i = 0; i < Screen_objects.size(); i++)
        {
            ...
            //the player swiches out to one with a powerup upgrade.
            {
                ...
                Screen_objects[i] = new RoF_Player(old_position);
                delete temp;
            }
        }
        return true;
